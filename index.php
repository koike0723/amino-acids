...
<body>
	...
	<main role="main" class="student-index-main" style="padding:60px 15px 0">
							...
							<?php foreach ($student['bookings'] as $booking): ?>
								<div class="student-index-flex">
									...
									<div class="student-index-flex-time">
										<?= h(substr($booking['cc_time'], 0, 5)); ?>
									</div>
									<div class="student-index-flex-style">
										<?= h($booking['cc_style_name']); ?>
									</div>
									<div class="student-index-flex-btn-area">
										<?php if ($booking['is_cc_plus'] === true): ?>
											<button
												...
												onclick="location.href='./student_reserve.php?selected_date=<?= h($booking['cc_date']); ?>&booking_id=<?= h($booking['booking_id']); ?>'">
												変更
											<?php else: ?>
												<button
													type="button"
													class="student-index-flex-btn-change"
													onclick="location.href='./student_cc_detail_require.php?selected_date=<?=h($booking['cc_date']); ?>&booking_id=<?= h($booking['booking_id']); ?>'">
													変更
												</button>
											<?php endif; ?>
									</div>
									<div class="student-index-flex-btn-area">
										...
										<button
											...
											onclick="location.href='./student_reserve_del_confirm.php?selected_date=<?= h($booking['cc_date']); ?>&time=<?= h($booking['cc_time']); ?>'" <?= $booking['is_cc_plus'] === false ? 'style="visibility: hidden;"' : ''; ?>>
											キャンセル
										</button>
									</div>
								</div>
		...
		<section class="student-index-section-ccplus">
			<p class="student-index-h1">キャリコンプラス実施日（新規予約）</p>
			<div class="cc-plus-list-area">
				<ul class="cc-plus-list-ul">
					<?php foreach ($cc_plus_table as $cc_plus): ?>
						<li class="cc-plus-list-li">
							<a href="./student_reserve.php?selected_date=<?= h($cc_plus['cc_date']); ?>"><?= h($cc_plus['cc_date']); ?></a>
						</li>
					<?php endforeach; ?>
				</ul>
			</div>
