...
<body>
	...
	<main class="container py-5">
		<section class="student-reservation-apply-section">
			<form action="./php_do/<?= h($booking_id) ? 'reserve_edit_do.php' : 'reserve_do.php' ?>">
					...
					<div class="col-12 col-md-8 col-lg-6 text-center">
						...
						<div class="student-reservation-apply-detail mx-auto">
							<div class="row mb-3">
								...
								<div class="col-8 text-start" style="text-align: left;"><?= h($student['course_name']); ?></div>
							</div>

							<div class="row mb-3">
								<div class="col-4 text-end fw-bold">名前：</div>
								<div class="col-8 text-start" style="text-align: left;"><?= h($student['student_name']); ?></div>
							</div>

							<div class="row mb-3">
								<div class="col-4 text-end fw-bold">日時：</div>
								<div class="col-8 text-start" style="text-align: left;"><?= h($selected_date); ?>&nbsp;<?= h($time); ?></div>
							</div>
												...
												<?php foreach ($cc_style as $style): ?>
													<div style="display: flex; gap: 5px; padding-inline-end: 20px;">
														<input
															...
															type="radio"
															name="style_id"
															id="student-method-face-<?= h($style['id']); ?>" style="padding-inline: 30px;"
															value="<?= h($style['id']); ?>" <?= $booking['style_id'] === $style['id'] ? 'checked' : '' ?>>

														<label class="form-check-label" for="student-method-face-<?= h($style['id']); ?>"><?= h($style['name']); ?></label>
													</div>
												<?php endforeach; ?>
											<?php else: ?>
												<?php foreach ($cc_style as $style): ?>
													<div style="display: flex; gap: 5px; padding-inline-end: 20px;">
														<input
															...
															name="style_id"
															id="student-method-face-<?= h($style['id']); ?>" style="padding-inline: 30px;"
															value="<?= h($style['id']); ?>" <?= $style['id'] === 1 ? 'checked' : '' ?>>
						...
						<div class="student-reservation-apply-buttons d-flex justify-content-center gap-3 mt-4">
							<input type="hidden" name="timeid" value="<?= h($timeId) ?>">
							<?php if ($booking_id): ?>
							...
							<button type="submit" class="btn btn-success student-reservation-apply-submit">
								<?= $booking_id ? '変更申請' : '追加申請'; ?>
							</button>
							<a href="student_reserve.php?selected_date=<?= h($selected_date); ?>&booking_id=<?= h($booking_id); ?>" class="btn btn-secondary">戻る</a>
						</div>
					</div>
				</div>
			</form>
