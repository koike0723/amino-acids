<!-- http://localhost:8080/amino-acids/php_do/course_edit_do.php -->
<!-- コース追加実行処理 -->
<?php
...
if (
    ...
    header("Location: ../admin_course_edit.php?status=error&message=no_data");
    exit;
}
...
if (strtotime($start_date) > strtotime($finish_date)) {
    header("Location: ../admin_course_edit.php?status=error&message=error_date");
    exit;
}
...
if (count($real_cc_days) != count(array_unique($real_cc_days))) {
    header("Location: ../admin_course_edit.php?status=error&message=error_cc_date");
    exit;
}
...
try {
    ...
    header("Location: ../admin_course_edit.php?status=error&message=cant_db");
    exit;
}
if ($same_check > 0) {
    header("Location: ../admin_course_edit.php?status=error&message=same_course");
    exit;
}
...
//     exit;
// }
// header("Location: ../admin_course_edit.php?status=success");
// exit;
// ?>
<pre>
    <?php var_dump($course) ?>
